...
weather_url += "&current_weather=true&windspeed_unit=mph"
...
pixels = neopixel.NeoPixel(board.A3, 5*5, brightness=float(os.getenv('BRIGHTNESS')), auto_write=False)
...
while True:
    ...
    time.sleep(float(os.getenv("SPRITE_TIME")))
    # draw scrolling text, if enabled
    if os.getenv('SHOW_SCROLLING_TEXT') == "TRUE":
        for v in range(2):
